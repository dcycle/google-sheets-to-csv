...
# Create a droplet
DROPLET_NAME=docker-google-sheets-to-csv-public
...
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@"$IP" "cd docker-drupal-job && ./scripts/install-docker-and-fetch-google-sheets.sh $1 $2 $3 $4"
